...
class DataLoading:
    ...
    def __init__(self):
        ...
        self.symbols_and_numbers_img_path = "Data/SymbolsAndNumbers"
        self.symbols_and_numbers_img_txt = "Data/numAndSymbols.txt"
    ...
    def prepare_symbols_and_numbers_data(self):
        symbols_and_numbers_images_paths_and_labels = []
        symbols_and_numbers_text = set()
        max_label_len = 0

        with open(self.symbols_and_numbers_img_txt, 'r', encoding='utf-8') as file:
            lines = file.readlines()

        for line in lines:
            parts = line.strip().split(' ', 1)
            image_name = parts[0] + ".png"
            folder_name = parts[0].split('_')[0]
            path = self.symbols_and_numbers_img_path + "/" + folder_name + "/" + image_name
            label = parts[1]
            max_label_len = max(max_label_len, len(label))
            symbols_and_numbers_text.update(list(label))
            symbols_and_numbers_images_paths_and_labels.append([path, label])

        return symbols_and_numbers_images_paths_and_labels, symbols_and_numbers_text, max_label_len
    ...
    def load_data(self):
        ...
        symbols_and_numbers_dataset, symbols_and_numbers_text, max_symbols_and_numbers_text_len = self.prepare_symbols_and_numbers_data()

        dataset = []
        dataset.extend(polish_dataset)
        dataset.extend(english_dataset)
        dataset.extend(symbols_and_numbers_dataset)
